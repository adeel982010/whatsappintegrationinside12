            ...
            for inv in self:
                ...
                if inv.company_id.invoice_order_information_in_message and inv.partner_id and inv.currency_id and inv.company_id:
                    ...
                    if inv.sequence_number_next_prefix and inv.sequence_number_next:
                        txt_message +=  "Here is the your invoice "+'*'+str(inv.sequence_number_next_prefix)+str(inv.sequence_number_next)+'*'+""
                    elif inv.number:
                        txt_message +=  "Here is the your invoice "+'*'+str(inv.number)+'*'+""
                    ...
                    if inv.invoice_line_ids:
                        for invoices_line in inv.invoice_line_ids:
                            if invoices_line.product_id:
                                txt_message +=  "%0A"+"*"+invoices_line.product_id.name+"*"+"%0A"+"*Qty:* "+str(invoices_line.quantity)+"%0A"+"*Price:* "+str(invoices_line.price_unit)+""+str(invoices_line.invoice_id.currency_id.symbol)+"%0A"
                            else:
                                txt_message +=  "%0A"+"*"+invoices_line.name+"*"+"%0A"+"*Qty:* "+str(invoices_line.quantity)+"%0A"+"*Price:* "+str(invoices_line.price_unit)+""+str(invoices_line.invoice_id.currency_id.symbol)+"%0A"
